/*
 * file_io, Task 3: program that copies the content of a file to another file
 */
#include "main.h"

/**
 * quit - prints error messages and exits with exit value
 * @error: num is either exit value or file descriptor
 * @s: str is a name, either of the two filenames
 * @fd: file descriptor
 * Return: 0 on success
 **/
int quit(int error, char *s, int fd)
{
	switch (error)
	{
	case 97:
...
int main(int argc, char *argv[])
{
	...
	close(fd_2) == -1 ? (quit(100, NULL, fd_2)) : close(fd_2);
	close(fd_1) == -1 ? (quit(100, NULL, fd_1)) : close(fd_1);
	return (0);
}
